...
namespace BddTools.AbstractSyntaxTrees {
    public static class BddMappedFormulaBuilder {
        ...
        public static bool CheckFormulaIsBddMapped(Formula f, out bool leafsNeedsRewrite) {
        ...
        public static void CheckFormulaIsBddMappedAndPatchIfNeeded(Formula formula) {
            var formulaIsBdd = CheckFormulaIsBddMapped(formula, out var leafsNeedRewrite);
            if (!formulaIsBdd) {
                throw new ArgumentException($"{formula} is not BDD-mapped formula.", nameof(formula));
            }
